...
public class ApplicationContextInitListener implements ServletContextListener {
    ...
    public void contextInitialized(ServletContextEvent ctxEvent)  { 
		...
		// tomcat aplication-context.xml 경로
		...
		try {
			...
			// bean 인스턴스를 생성해서 Map에 주입하는 과정
			for (int i = 0; i < beans.getLength(); i++) {
				Node bean = beans.item(i);
				if (bean.getNodeType() == Node.ELEMENT_NODE) { // 요소 노드인지 확인??? 요소 노드가 뭐임???
						...
						if(property.getNodeType() == Node.ELEMENT_NODE) {
							...
							if(methods != null) for (Method method : methods) {
								...
								if (method.getName().equals(setMethodName)) {
									method.invoke(applicationContext.get(
											eleBean.getAttribute("id")), 
